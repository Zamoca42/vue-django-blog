  ...
  <v-container fluid style="width: 1200px">
    <v-row class="mb-6" style="height: 200px" align="center" justify="center">
      ...
      <v-col cols="12" lg="12" align="center">
        <v-btn variant="text" v-for="(cate, index) in cateList" :key="index" @click="categoryPage(cate.text)">
          {{ cate.text }}
        </v-btn>
      </v-col>
    </v-row>
    ...
    <v-row cols="12" lg="10">
      <v-col v-for="post in postList" :key="post.id" lg="3" md="4" sm="6" cols="12">
        <v-card elevation="0" @click="serverPage(post.id)" :ripple="false" class="rounded-xl fill-height">
          <template v-if="post.image === null">
            <v-img :src="defaultImageUrl" class="rounded-xl" style="height: 150px" cover></v-img>
          </template>
          <template v-else>
            <v-img :src="post.image" class="rounded-xl" style="height: 150px" cover></v-img>
          </template>
          <v-card-text>
            <v-row>
          ...
          <v-chip v-for="(tag, index) in post.tags" :key="index" class="ml-2 mb-3 text-disabled" size="x-small">
            {{ tag }}
          </v-chip>
        </v-card>
      </v-col>
    ...
    <v-row align="center" justify="center">
      <div class="text-center">
        ...
        <template v-else>
          <v-pagination v-model="page" :length="pageCnt" :total-visible="4" rounded="circle"
  ...
  methods: {
    async fetchPostList(page = 1) {
      ...
      try {
        ...
        this.pageCnt = response.data.pageCnt;
